...
!(async () => {
  ...
  if (exchangeFlag) {
    ...
    console.log(`脚本不会自动抽奖，建议活动快结束开启，默认关闭(在11.2日自动开启抽奖),如需自动抽奖请设置环境变量  JD_CITY_EXCHANGE 为true`);
  }
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://xr2021.coding.net/p/import-kasd/d/JDbot/git/raw/master/shareCodes/city.json`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
